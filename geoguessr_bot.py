...
PROMPT_INSTRUCTIONS = """
I will present you with an image of the game Geoguessr, and you will 
try to guess the exact coordinates of where the picture has been taken.
First describe the relevant details in the image to do it.
List some regions and places where it could be. Eliminate regions if they are not in Google Street View, 
such as Tanzania, Mozambique, Saudi Arabia, Burkina Faso, Bosnia, and Mali.
Chose the most likely City or road of the location.
At the end, in the last line a part from the previous reasoning, write the Latitude and 
Longitude from that guessed location using the following format, making sure that the 
coords are valid floats, without anything else and making sure to be consistent with 
the format: Lat: XX.XXXX, Lon: XX.XXXX
"""
...
class GeoBot:
    ...
    def __init__(self, screen_regions, player=1, model=ChatGoogleGenerativeAI, model_name="gemini-1.5-flash", prompt_instructions=PROMPT_INSTRUCTIONS):
        self.player = player
    ...
    def select_map_location(self, x: int, y: int, plot: bool = False) -> None:
        ...
        pyautogui.moveTo(self.screen_bot_right_x, self.screen_bot_right_y)
        sleep(2)
